...
class ApiCn:
    ...
    def request(self, **kw):
        ...
        headers = {"Content-type": "application/x-www-form-urlencoded", "Accept": "text/json",
                   "User-Agent": "dnspod-python/0.01 (chenliang@haizhi.com; DNSPod.CN API v2.8)"}
        # conn.request("POST", self.path, urllib.parse.urlencode(self.params), headers)
...
class RecordDdns(_DomainApiBase):
    def __init__(self, record_id, sub_domain, record_type, record_line, **kw):
        kw.update(dict(
            record_id=record_id,
            sub_domain=sub_domain,
            record_type=record_type,
